...
func aboutMaps() {
	...
	age, ok := ages["bob"]
	assert(ok) // with a handy multiple-assignment variation

	age, ok = ages["steven"]
	assert(age == 0) // the zero value is used when absent
	assert(!ok)      // though there are better ways to check for presence
	...
	delete(ages, "steven")
	age, ok = ages["steven"]
	assert(!ok) // key/value pairs can be removed
}
